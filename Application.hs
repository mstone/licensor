...
mkYesodDispatch "Licensor" resourcesLicensor
...
withLicensor :: AppConfig -> Logger -> (Application -> IO a) -> IO ()
withLicensor conf logger f = do
    s <- static Settings.staticDir
    Settings.withConnectionPool conf $ \p -> do
        runConnectionPool (runMigration migrateAll) p
        let h = Licensor conf logger s p
#ifdef WINDOWS
        toWaiApp h >>= f >> return ()
#else
...
withDevelAppPort =
  ...
  where
    ...
    go f = do
        ...
        withLicensor conf logger $ \app -> f (port, debugHandle (logHandle logger) app)
        flushLogger logger
      where
